...
type ProductImageFile = {
    file: File;
    imageId?: string; // будет, если это изображение уже есть на сервере
};
...
function Edit() {
    ...
    const [mainImage, setMainImage] = useState<ProductImageFile | null>(null);
    ...
    const handleProductImagesChange = () => {
        ...
        if (files) {
            ...
            setProductImages(prev => [...prev, ...fileArray.map((file) => {
                return {
                    file: file
                }
            })]);
    ...
    const handleDelete = async () => {
        setLoading(true)
        await api.get<ApiResponse<Product>>('/products/delete/' + params.id, {
            headers: {
    ...
    const fetchProduct = async () => {
        ...
        try {
            const response = await api.get<ApiResponse<Product>>('/products/id/' + params.id);
            ...
            setName(fetched.title);
            setPrice(fetched.price.toString());
            setDescription(fetched.description);

            setDelivery(fetched.delivery);
            setMinCount(fetched.minCount.toString());


            // Категория и подкатегория
            setSelectedCategory(fetched.category);
            setSelectedSubCategory(fetched.subCategory);

            setCity(fetched.city);

            // Харектеристики
            if (fetched.variables) {
                const restoredVars: { [key: string]: string } = {};
                for (const variable of fetched.variables) {
                    restoredVars[variable.name] = variable.value;
                }
                setVariables(restoredVars);
            }

            // Изображения (здесь ты можешь просто сохранить blob или ссылки, если есть)
            if (fetched.banner) {
                try {
                    const blob = await fetch('https://api.sedmarket.kz/v1/images/' + fetched.banner).then(r => r.blob());
                    const file = new File([blob], "main.jpg", { type: blob.type });
                    setMainImage({
                        file: file,
                        imageId: fetched.banner
                    });
                } catch {
                    console.error("Не удалось загрузить главное изображение");
                }
            }

            if (fetched.images && fetched.images.length > 0) {
                try {
                    const files: ProductImageFile[] = await Promise.all(
                        fetched.images.map(async (imageId: string, i: number) => {
                            const blob = await fetch(`https://api.sedmarket.kz/v1/images/${imageId}`).then(r => r.blob());
    ...
    const buildFormData = async () => {
        ...
        formData.append("minCount", String(minCount));
        ...
        formData.append("mainImage", mainImage ? mainImage.file : '');
        formData.append("mainImageIds", mainImage.imageId ? mainImage.imageId : '');

        productImages.forEach((imgObj) => {
            formData.append("productImages", imgObj.file);
            if (imgObj.imageId) {
                formData.append("productImageIds", imgObj.imageId);
            }
        });
            ...
            <div className="flex flex-wrap sm:flex-nowrap gap-12">
                <div className="min-w-72">
                    ...
                    <div className="mt-6 flex flex-col gap-2">
                        ...
                        <div
                            className="bg-red-500 rounded-2xl text-white py-2 flex justify-center items-center gap-3 font-medium cursor-pointer"
                            ...
                            <EditForm text="На главной странице">
                                <div className="flex flex-col gap-3">
                                    ...
                                    <div className="w-full aspect-square rounded-xl overflow-hidden bg-muted flex items-center justify-center">
                                        {mainImage ? (
                                            <img src={URL.createObjectURL(mainImage.file)} alt="Предпросмотр" className="w-full h-full object-cover" />
                                        ) : (
                            ...
                            <div className="mt-3 grid grid-cols-3 gap-3">
                                {productImages.map((image, index) => (
                                    <div key={index} className="relative w-full h-32 bg-muted rounded-2xl overflow-hidden">
                                        <img src={URL.createObjectURL(image.file)} alt="Product Image" className="object-cover w-full h-full" />
                                        <div
