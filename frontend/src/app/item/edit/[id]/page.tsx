...
function Edit() {
    ...
    const fetchProduct = async () => {
        ...
        try {
            ...
            if (fetched.banner) {
                try {
                    const blob = await fetch('http://localhost:3001/v1/images/' + fetched.banner).then(r => r.blob());
                ...
                try {
                    const files: ProductImageFile[] = await Promise.all(
                        fetched.images.map(async (imageId: string, i: number) => {
                            const blob = await fetch(`http://localhost:3001/v1/images/${imageId}`).then(r => r.blob());
